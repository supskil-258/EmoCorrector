...
class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, shuffle):
        ...
        self.clean_sizes = None

    @property
    def _clean_sizes(self):
        return self.clean_sizes
    ...
    def __len__(self):
        return len(self._clean_sizes)

    def num_tokens(self, index):
        return self.clean_size(index)

    def clean_size(self, index):
        """Return an example's size as a float or tuple. This value is used when
        filtering a dataset with ``--max-positions``."""
        return min(self._clean_sizes[index], hparams['max_frames'])

    def ordered_indices(self):
        """Return an ordered list of indices. Batches will be constructed based
        on this order."""
        if self.shuffle:
            clean_indices = np.random.permutation(len(self._clean_sizes))
            if self.sort_by_len:
                clean_indices = clean_indices[np.argsort(np.array(self._clean_sizes)[clean_indices], kind='mergesort')]
        else:
            clean_indices = np.arange(len(self._clean_sizes))
...
class BaseTask(nn.Module):
    ...
    def training_step(self, sample, batch_idx, optimizer_idx=-1):
        ...
        """
        #########################################################
            New added values to tb_outputs and log_outputs 
        #########################################################
        """

        log_outputs['clean_emo2emo_acc'] = model_out['clean_emo2emo_acc']
        log_outputs['clean_spk2spk_acc'] = model_out['clean_spk2spk_acc']
        #log_outputs['pred_emo2emo_acc'] = model_out['pred_emo2emo_acc']
        log_outputs['clean_spk2emo_acc'] = model_out['clean_spk2emo_acc']

        log_outputs['loss_CLAP'] = model_out.get('loss_CLAP')#,0)
        log_outputs['mel_loss'] = model_out.get('mel_loss', 0)
        log_outputs['pred_op_loss_emo2emo'] = model_out.get('pred_op_loss_emo2emo', 0)
        log_outputs['pred_op_loss_spk2spk'] = model_out.get('pred_op_loss_spk2spk', 0)

        log_outputs['clean_loss_grl_spk2emo'] = model_out.get('clean_loss_grl_spk2emo', 0)
        log_outputs['clean_loss_grl_emo2spk'] = model_out.get('clean_loss_grl_emo2spk', 0)
        # log_outputs['emo_vq_perplexity'] = model_out['emo_vq_perplexity'].item()


        # log_outputs['noisy_emo2emo_acc'] = model_out['noisy_emo2emo_acc']

        log_outputs['total_loss'] = total_loss.item()
