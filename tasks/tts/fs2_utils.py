...
class FastSpeechDataset(BaseDataset):
    def __init__(self, prefix, shuffle=False):
        ...
        # pitch stats
        clean_f0_stats_fn = f'{self.data_dir}/train_clean_f0s_mean_std.npy'
        noisy_f0_stats_fn = f'{self.data_dir}/train_noisy_f0s_mean_std.npy'
        if os.path.exists(clean_f0_stats_fn):
            hparams['clean_f0_mean'], hparams['clean_f0_std'] = self.clean_f0_mean, self.clean_f0_std = np.load(clean_f0_stats_fn)
            hparams['clean_f0_mean'] = float(hparams['clean_f0_mean'])
            hparams['clean_f0_std'] = float(hparams['clean_f0_std'])
        else:
            hparams['clean_f0_mean'], hparams['clean_f0_std'] = self.clean_f0_mean, self.clean_f0_std = None, None

        if os.path.exists(noisy_f0_stats_fn):
            hparams['noisy_f0_mean'], hparams['noisy_f0_std'] = self.noisy_f0_mean, self.noisy_f0_std = np.load(noisy_f0_stats_fn)
            hparams['noisy_f0_mean'] = float(hparams['noisy_f0_mean'])
            hparams['noisy_f0_std'] = float(hparams['noisy_f0_std'])
        else:
            hparams['noisy_f0_mean'], hparams['noisy_f0_std'] = self.noisy_f0_mean, self.noisy_f0_std = None, None
