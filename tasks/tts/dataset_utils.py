...
from utils.pitch_utils import clean_norm_interp_f0
...
class BaseTTSDataset(BaseDataset):
    def __init__(self, prefix, shuffle=False, test_items=None, test_sizes=None, data_dir=None):
        ...
        def load_size():
            self.clean_sizes = np.load(f'{self.data_dir}/{self.prefix}_clean_lengths.npy')
            #self.noisy_sizes = np.load(f'{self.data_dir}/{self.prefix}_noisy_lengths.npy')

        if prefix == 'test':
            if test_items is not None:
                self.clean_indexed_ds, self.clean_sizes = test_items, test_sizes
                #self.noisy_indexed_ds, self.noisy_sizes = test_items, test_sizes
            else:
                load_size()
            if hparams['num_test_samples'] > 0:
                self.clean_avail_idxs = [x for x in range(hparams['num_test_samples']) \
                                   if x < len(self.clean_sizes)]
                # self.noisy_avail_idxs = [x for x in range(hparams['num_test_samples']) \
                #                          if x < len(self.noisy_sizes)]
                if len(hparams['test_ids']) > 0:
                    self.clean_avail_idxs = hparams['test_ids'] + self.clean_avail_idxs
                    # self.noisy_avail_idxs = hparams['test_ids'] + self.noisy_avail_idxs
            else:
                self.clean_avail_idxs = list(range(len(self.clean_sizes)))
                # self.noisy_avail_idxs = list(range(len(self.noisy_sizes)))
        else:
            load_size()
            self.clean_avail_idxs = list(range(len(self.clean_sizes)))
            # self.noisy_avail_idxs = list(range(len(self.noisy_sizes)))
        ...
        if hparams['min_frames'] > 0:
            self.clean_avail_idxs = [
                x1 for x1 in self.clean_avail_idxs if self.clean_sizes[x1] >= hparams['min_frames']]
            # self.noisy_avail_idxs = [
            #     x2 for x2 in self.noisy_avail_idxs if self.noisy_sizes[x2] >= hparams['min_frames']]
        self.clean_sizes = [self.clean_sizes[i] for i in self.clean_avail_idxs]
        # self.noisy_sizes = [self.noisy_sizes[i] for i in self.noisy_avail_idxs]
    ...
    def collater(self, samples):
        ...
        # 批量处理情感嵌入
        if hparams['use_emo_embed']:
            emo_embeddings = torch.stack([s['emo_embedding'] for s in samples])
            batch['emo_embedding'] = emo_embeddings
        return batch
...
class FastSpeechDataset(BaseTTSDataset):
    ...
    def __getitem__(self, index):
        ...
        if hparams['use_pitch_embed']:
            ...
            f0, uv = clean_norm_interp_f0(item["f0"][:max_frames], hparams)
            ...
            if hparams['pitch_type'] == 'cwt':
                ...
                if "f0_ph" in item:
                    ...
                    f0 = clean_denorm_f0(f0, None, hparams)
                ...
                f0, uv = clean_norm_interp_f0(f0_ph, hparams)
        else:
