...
class FastSpeech2Task(TTSBaseTask):
    def __init__(self):
        ...
        clean_f0_stats_fn = f'{hparams["binary_data_dir"]}/train_clean_f0s_mean_std.npy'
        noisy_f0_stats_fn = f'{hparams["binary_data_dir"]}/train_noisy_f0s_mean_std.npy'
        if os.path.exists(clean_f0_stats_fn):
            hparams['clean_f0_mean'], hparams['clean_f0_std'] = np.load(clean_f0_stats_fn)
            hparams['clean_f0_mean'] = float(hparams['clean_f0_mean'])
            hparams['clean_f0_std'] = float(hparams['clean_f0_std'])
        if os.path.exists(noisy_f0_stats_fn):
            hparams['noisy_f0_mean'], hparams['noisy_f0_std'] = np.load(clean_f0_stats_fn)
            hparams['noisy_f0_mean'] = float(hparams['noisy_f0_mean'])
            hparams['noisy_f0_std'] = float(hparams['noisy_f0_std'])
    ...
    def _training_step(self, sample, batch_idx, _):
        ...
        loss_output, outputs = self.run_model(self.model,batch_idx, sample)
        total_loss = sum([v for v in loss_output.values() if isinstance(v, torch.Tensor) and v.requires_grad])
        loss_output['batch_size'] = sample['txt_tokens'].size()[0]
        return total_loss, loss_output, outputs
    ...
    def plot_dur(self, batch_idx, sample, model_out):
        ...
        # 需要修改
        dur_gt = mel2ph_to_dur(sample['clean_mel2ph'], T_txt)[0]
    ...
    def plot_pitch(self, batch_idx, sample, model_out):
        ...
        self.logger.add_figure(
            f'f0_{batch_idx}',
            f0_to_figure(model_out['clean_f0_denorm'][0], None, model_out['clean_f0_denorm_pred'][0]),
            self.global_step)
    ...
    def test_step(self, sample, batch_idx):
        ...
        if hparams['profile_infer']:
            ...
            if hparams['use_pitch_embed']:
                sample['f0'] = clean_denorm_f0(sample['f0'], sample['uv'], hparams)
