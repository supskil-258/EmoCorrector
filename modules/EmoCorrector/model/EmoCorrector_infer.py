...
class EmoCorrector(FastSpeech2):
	...
	def forward(self, text = None, txt_tokens=None, spk_id=None, emotion_id=None, mel2ph=None, ref_mel2ph=None, ref_mel2word=None,
		...
		"""
		#########################################################
			Teacher-Emotion-emedding Extractor and Teacher-Speaker-embedding Extractor
				1. Teacher Mel Style Encoder: local[B,T,H]; global[B,H];
				2. MBV
				3. emotion/speaker classifer
		#########################################################
		"""
		# 1. Use Mel-Style-Encoder get representation
		'''
			local: B,T,H
			global: B,1,H
		'''
		...
		emo_embedding = self.audio_adaptor(emo_embedding)
		pred_emo_embed = self.emo_fc(emo_embedding.unsqueeze(0)).to(device)
		...
		pitch_inp_domain_specific = (
												decoder_inp + pred_spk_embed + pred_emo_embed) * tgt_nonpadding2  # +spk_prosody_utter_mel+spk_prosody_ph_mel+spk_prosody_word_mel
		predicted_pitch = self.inpaint_pitch(pitch_inp_domain_agnostic, pitch_inp_domain_specific, f0, uv, mel2ph,
											 ret)
		# decode
		# decoder_inp = txt + spk_embedding + emo_embedding + pitch + local_style(utter + ph + word)
		decoder_inp = decoder_inp + pred_spk_embed + pred_emo_embed + predicted_pitch  # + spk_prosody_utter_mel+spk_prosody_ph_mel+spk_prosody_word_mel
