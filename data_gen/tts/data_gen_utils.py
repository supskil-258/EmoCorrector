...
def get_mel2ph(tg_fn, ph, mel, hparams):
    ...
    while tg_idx < len(tg_align) or ph_idx < len(ph_list):
        ...
        ph = ph_list[ph_idx]
        # # 过滤掉空标记（静音音素）并确保与 ph_list 的处理一致
        # tg_align = [x for x in tg_align if x['text'] != '']
        # for x in tg_align:
        #     print(f"Phoneme: {x['text']}, xmin: {x['xmin']}, xmax: {x['xmax']}")
        #
        # print(f"tg_align (filtered): {[x['text'] for x in tg_align]}")
        # print(f"ph_list: {ph_list}")
        # print(f"tg_align details: {tg_align}")
        # print(f"ph_list length: {len(ph_list)}, tg_align length: {len(tg_align)}")
