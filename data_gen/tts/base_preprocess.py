...
from utils.os_utils import link_file, move_file, remove_file, copy_file
...
class BasePreprocessor:
    ...
    def process(self):
        ...
        clean_wav_processed_tmp_dir = f'{processed_dir}/processed_tmp/clean'
        remove_file(clean_wav_processed_tmp_dir)
        os.makedirs(clean_wav_processed_tmp_dir, exist_ok=True)
        clean_wav_processed_dir = f'{processed_dir}/{self.wav_processed_dirname}/clean'
        remove_file(clean_wav_processed_dir)
        os.makedirs(clean_wav_processed_dir, exist_ok=True)
        ...
        process_item = partial(self.preprocess_first_pass,
                               ...
                               clean_wav_processed_dir=clean_wav_processed_dir,
                               clean_wav_processed_tmp=clean_wav_processed_tmp_dir,
                               preprocess_args=self.preprocess_args)
        items = []
        args = [{
            'item_name': item_raw['item_name'],
            'txt_raw': item_raw['txt'],
            'clean_wav_fn': item_raw['clean_wav_fn'],
        ...
        if self.preprocess_args['use_mfa']:
            clean_mfa_dict = set()
            clean_mfa_input_dir = f'{processed_dir}/mfa_inputs/clean'
            remove_file(clean_mfa_input_dir)
            ...
            args = [{
                'item': item,
                'clean_mfa_input_dir': clean_mfa_input_dir,
                'mfa_group': mfa_group,
                'clean_wav_processed_tmp': clean_wav_processed_tmp_dir,
                'preprocess_args': self.preprocess_args
            } for item, mfa_group in zip(items, mfa_groups)]
            for i, (ph_gb_word_nosil, clean_new_wav_align_fn) in multiprocess_run_tqdm(
                    self.build_mfa_inputs, args, desc='Build MFA data'):
                items[i]['clean_wav_align_fn'] = clean_new_wav_align_fn
                for w in ph_gb_word_nosil.split(" "):
                    clean_mfa_dict.add(f"{w} {w.replace('_', ' ')}")
            clean_mfa_dict = sorted(clean_mfa_dict)
            with open(f'{processed_dir}/clean_mfa_dict.txt', 'w') as f:
                f.writelines([f'{l}\n' for l in clean_mfa_dict])
    ...
    def preprocess_first_pass(cls, item_name, txt_raw, txt_processor,
                              clean_wav_fn,
                              clean_wav_processed_dir,
                              clean_wav_processed_tmp,
        ...
        try:
            ...
            clean_wav_fn, clean_wav_align_fn = cls.process_wav(
                item_name, clean_wav_fn,
                hparams['processed_data_dir'],
                clean_wav_processed_tmp, preprocess_args)
            # wav for binarization
            ext = os.path.splitext(clean_wav_fn)[1]
            os.makedirs(clean_wav_processed_dir, exist_ok=True)
            clean_new_wav_fn = f"{clean_wav_processed_dir}/{item_name}{ext}"
            checka = os.path.dirname(clean_wav_fn)
            checkb = clean_wav_processed_tmp
            clean_move_link_func = move_file if os.path.dirname(clean_wav_fn) == clean_wav_processed_tmp else copy_file

            clean_move_link_func(clean_wav_fn, clean_new_wav_fn)
            return {
                'txt': txt, 'txt_raw': txt_raw, 'ph': ph,
                'word': word, 'ph2word': ph2word, 'ph_gb_word': ph_gb_word,
                'clean_wav_fn': clean_new_wav_fn,
                'clean_wav_align_fn': clean_wav_align_fn,
                'others': others
            }
        except:
            traceback.print_exc()
    ...
    def build_mfa_inputs(cls, item, clean_mfa_input_dir, mfa_group, clean_wav_processed_tmp, preprocess_args):
        ...
        clean_mfa_input_group_dir = f'{clean_mfa_input_dir}/{mfa_group}'
        os.makedirs(clean_mfa_input_group_dir, exist_ok=True)
        clean_new_wav_align_fn = f"{clean_mfa_input_group_dir}/{item_name}{ext}"
        clean_move_link_func = move_file if os.path.dirname(clean_wav_align_fn) == clean_wav_processed_tmp else copy_file
        ...
        with open(f'{clean_mfa_input_group_dir}/{item_name}.lab', 'w') as clean_f_txt:
            clean_f_txt.write(ph_gb_word_nosil)
        return ph_gb_word_nosil, clean_new_wav_align_fn
